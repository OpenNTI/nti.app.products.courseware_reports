...
def _get_course_ntiids(instance):
    ...
    # We want to check our parent course for refs in the outline, if we
    # have shared outlines.
...
def _get_self_assessments_for_course(course):
    ...
    in the course.
    """
    # Gather refs from the outline
    result = set()
    catalog = get_library_catalog()
    course_ntiids = _get_course_ntiids(course)
    sites = get_component_hierarchy_names()
    for obj in catalog.search_objects(provided=(INTIQuestionSetRef,),
                                      container_ntiids=course_ntiids,
                                      sites=sites,
                                      container_all_of=False):
        qset = find_object_with_ntiid(obj.target)
        if qset is not None:
            result.add(qset)
    return tuple(result)
...
class TopCreators(object):
    ...
    def _do_get_largest(self, data, total_to_change):
        ...
        if len(data) > len(largest):
            ...
            self.aggregate_remainder = StudentInfo( username=u'Others',
                                                    display=u'Others',
                                                    count=largest_total,
                                                    perc=percent )
        return largest
    ...
    @Lazy
    def series(self):
        return u' '.join( (u'%d' % x.count for x in self._get_largest ) )
    ...
    def average_count_str(self):
        return u"%0.1f" % self.average_count()
    ...
    def percent_contributed_str(self):
        return u"%0.1f" % self.percent_contributed( self.max_contributors, self.unique_contributors )

    def for_credit_percent_contributed_str(self):
        return u"%0.1f" % self.percent_contributed( self.max_contributors_for_credit, self.unique_contributors_for_credit )

    def non_credit_percent_contributed_str(self):
        return u"%0.1f" % self.percent_contributed( self.max_contributors_non_credit, self.unique_contributors_non_credit )
...
def _build_buckets_options(options, buckets):
    ...
    if forum_objects_by_week_number:
        ...
        def as_series():
            rows = [u'%d' % forum_objects_by_week_number.get(k, 0)
                    for k in full_range]
            return u'\n'.join(rows)
        ...
        if len( buckets.group_dates ) < 13:
            for d_entry in buckets.group_dates:
                weeks_s.append( d_entry.strftime( u'%b %d' ) )
        else:
            for d_entry in buckets.group_dates:
                weeks_s.append( d_entry.strftime( u'%m-%d' ) )
    ...
    for i, q in enumerate( ordered_questions ):
        ...
        for question_part_stat in question_part_stats.values():
            ...
            if question_part_stat.assessed_values:
                ...
                avg_assessed_s = u'%0.1f' % avg_assessed
        ...
        question_avg_assessed_s = u'%0.1f' % average( question_part_grades ) if question_part_grades else u'N/A'
...
def _finalize_answer_stats( answer_stats, total_submits ):
    ...
    for j in range( len( answer_stats ) ):
        ...
        sub.perc_s = u'%0.1f' % ( sub.count * 100.0 / total_submits ) if total_submits else u'N/A'
